...
import WebAdModal from '../components/WebAdModal';
...
export function GameProvider({ children }) {
  ...
  // Web Active Screen: Start at ONBOARDING (Login choice: Google or Demo) or CATALOG if authenticated
  ...
  // Resume AudioContext on iOS/Safari when app returns from background
  useEffect(() => {
  ...
  const addFriend = (friendInput) => {
    if (!friendInput || !friendInput.trim()) return false;
    const cleanInput = friendInput.trim();
    let isAdded = false;
    setUser(prev => {
      const currentFriends = prev.friends || [];
      if (currentFriends.includes(cleanInput)) return prev;
      isAdded = true;
      return {
        ...prev,
        friends: [...currentFriends, cleanInput]
      };
    });
    return isAdded;
  };

  const removeFriend = (friendInput) => {
    setUser(prev => ({
      ...prev,
      friends: (prev.friends || []).filter(f => f !== friendInput)
    }));
  };
