...
export function GameProvider({ children }) {
  ...
  const restoreUserProfileData = async (email, initialMeta = {}) => {
    ...
    // Badge: unione cloud + locale (union, nessuna perdita)
    const cloudUnlocked = Array.isArray(cloud?.unlockedBadges) ? cloud.unlockedBadges : [];
    const localUnlocked = Array.isArray(withLocal.unlockedBadges) ? withLocal.unlockedBadges : [];
    const finalUnlockedBadges = Array.from(new Set([...cloudUnlocked, ...localUnlocked]));

    const cloudStreak = Array.isArray(cloud?.streakBadges) ? cloud.streakBadges : [];
    const localStreak = Array.isArray(withLocal.streakBadges) ? withLocal.streakBadges : [];
    const finalStreakBadges = Array.from(new Set([...cloudStreak, ...localStreak]));
    ...
    const finalUser = {
      ...
      unlockedBadges: finalUnlockedBadges,
      streakBadges: finalStreakBadges,
      hasCompletedCalibration: true
    };
  ...
  // Memorizza il target di navigazione da un tap su notifica quando l'app era chiusa.
  // Viene applicato subito dopo il completamento dell'autenticazione.
  const pendingNotificationNav = useRef(null);

  // Deduplicazione eventi e notifiche (challenge_completed, challenge_received, ecc.):
  // previene doppi toast tra canali realtime (broadcast/postgres_changes) e push FCM in foreground.
  const notifiedChallengeCompletions = useRef(new Set());

  const applyPendingNotificationNav = () => {
    const pending = pendingNotificationNav.current;
    if (!pending) return;
    pendingNotificationNav.current = null;
    setActiveScreen('CHALLENGE');
    // Se c'è un challengeId specifico, lo memorizziamo per aprire quella sfida
    if (pending.challengeId) {
      // Salva in localStorage per lettura da ChallengeScreen al mount
      try {
        localStorage.setItem('ten_seconds_pending_challenge', JSON.stringify({
          challengeId: pending.challengeId,
          screen: pending.screen || 'challenges',
          ts: Date.now()
        }));
      } catch(e) {}
    }
  ...
  useEffect(() => {
    initPushNotifications({
      ...
      onNotificationReceived: (notification) => {
        ...
        const screen = data.screen || 'challenges';
        const rawId = data.challengeId || data.tournamentId || data.requestId || null;
        const cId = rawId ? String(rawId) : null;

        // Deduplicazione specifica basata sull'ID evento:
        // Sopprimi la notifica toast solo se l'evento per questo specifico ID è già arrivato
        // tramite broadcast / postgres_changes. Se non è ancora nel Set (canale disconnesso,
        // ritardo di rete o broadcast assente), mostra regolarmente il toast e registra il cId
        // nel Set per prevenire un secondo avviso duplicato qualora il broadcast arrivi successivamente.
        if (cId) {
          if (notifiedChallengeCompletions.current.has(cId)) {
            console.log('[PushNotifications] Foreground push skipped (already notified for id):', cId, nType);
            return;
          }
          notifiedChallengeCompletions.current.add(cId);
          setTimeout(() => notifiedChallengeCompletions.current.delete(cId), 30000);
        }
      ...
      onNotificationActionPerformed: (notification) => {
        // Leggi i dati dalla notifica (Capacitor: notification.notification.data)
        const data = notification?.notification?.data || notification?.data || {};
        const screen = data.screen || 'challenges';
        const challengeId = data.challengeId || data.tournamentId || null;

        console.log('[PushNotifications] Tap su notifica — screen:', screen, '| challengeId:', challengeId);

        if (user?.email) {
          // App in background: utente già autenticato, naviga subito
          setActiveScreen('CHALLENGE');
          if (challengeId) {
            try {
              localStorage.setItem('ten_seconds_pending_challenge', JSON.stringify({
          ...
          // App completamente chiusa: salva il target, verrà applicato post-login
          pendingNotificationNav.current = { screen, challengeId };
        }
      }
    }).catch(() => {});
  }, []);
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (data?.session?.user) {
        ...
        if (email) {
          ...
          // Applica navigazione pending da tap notifica (app era chiusa)
          setTimeout(() => applyPendingNotificationNav(), 600);
        }
      }
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      if (session?.user) {
        ...
        if (email) {
          ...
          if (event === 'SIGNED_IN') {
            setActiveScreen('CATALOG');
            // Applica navigazione pending da tap notifica dopo login completato
            setTimeout(() => applyPendingNotificationNav(), 600);
          }
        }
  ...
  useEffect(() => {
    ...
    const inboxSub = subscribeToUserInbox(user, {
      onFriendRequestReceived: (newReq) => {
        ...
        const reqId = newReq?.id ? String(newReq.id) : null;
        if (reqId) {
          if (notifiedChallengeCompletions.current.has(reqId)) return;
          notifiedChallengeCompletions.current.add(reqId);
          setTimeout(() => notifiedChallengeCompletions.current.delete(reqId), 30000);
        }
      ...
      onChallengeReceived: (newChallenge) => {
        ...
        // Deduplicazione toast: se la notifica per questa sfida è già stata mostrata dalla push FCM
        const cId = newChallenge?.id ? String(newChallenge.id) : null;
        if (cId) {
          if (notifiedChallengeCompletions.current.has(cId)) return;
          notifiedChallengeCompletions.current.add(cId);
          setTimeout(() => notifiedChallengeCompletions.current.delete(cId), 30000);
        }
      ...
      onChallengeCompleted: (resultPayload) => {
        const rawId = resultPayload?.challengeId;
        const cId = rawId ? String(rawId) : null;

        // 1. Aggiorna sempre lo stato locale delle sfide (operazione idempotente)
        ...
        // 2. Deduplicazione toast: se l'avviso per questo challengeId è già stato mostrato
        // (dalla push FCM in foreground o da un canale realtime precedente)
        if (cId) {
          if (notifiedChallengeCompletions.current.has(cId)) return;
          notifiedChallengeCompletions.current.add(cId);
          // Auto-clean dopo 30s per non accumulare memoria
          setTimeout(() => notifiedChallengeCompletions.current.delete(cId), 30000);
        }
      ...
      onTournamentReceived: (newTourn) => {
        ...
        if (tId) {
          ...
          if (notifiedChallengeCompletions.current.has(tId)) return;
          notifiedChallengeCompletions.current.add(tId);
          setTimeout(() => notifiedChallengeCompletions.current.delete(tId), 30000);
        }
      ...
      onTournamentUpdated: (updatedTourn) => {
        ...
        if (updatedTourn.status === 'completed' && updatedTourn.winner?.name) {
          const tId = updatedTourn?.id ? String(updatedTourn.id) : null;
          if (tId) {
            if (notifiedChallengeCompletions.current.has(tId)) return;
            notifiedChallengeCompletions.current.add(tId);
            setTimeout(() => notifiedChallengeCompletions.current.delete(tId), 30000);
  ...
  const createAsyncTournament = async (arg1, playlistArg, songCountArg = 10, invitedFriendsArg = []) => {
    let name, playlist, songCount = 10, invitedFriends = [], customTracks = null;
    if (arg1 && typeof arg1 === 'object' && !arg1.tracks && !arg1.id && !arg1.title) {
      ({ name, playlist, songCount = 10, customTracks = null, invitedFriends = [] } = arg1);
    } else {
      name = arg1;
      playlist = playlistArg;
      songCount = songCountArg;
      invitedFriends = invitedFriendsArg;
    }
  ...
  const endGame = async () => {
    ...
    if (pendingInboxResponseChallengeRef.current) {
      ...
      supabase.functions.invoke('submit-challenge-score', {
        body: {
          ...
          totalTimeMs: stats.totalTimeMs,
          maxStreak: maxStreak || 0,
          songCount: respCh.songCount || respCh.song_count || 10,
          clientScore: finalGameScore,
        }
    ...
    if (pendingTournamentMatchRef.current || gameMode === 'TOURNAMENT') {
      ...
      if (activeTourn) {
        const payloadBody = {
          ...
          totalTimeMs: stats.totalTimeMs,
          maxStreak: maxStreak || 0,
          songCount: activeTourn.songCount || activeTourn.song_count || 10,
          clientScore: finalGameScore,
        };
    ...
    // Update global user stats on Supabase if user is logged in
    if (user.id && user.id.length > 10) {
      const statsDelta = {
        total_score: finalGameScore,
        total_games_played: 1,
        total_songs_guessed: stats.correct,
        total_wrong_answers: stats.wrong,
        measured_correct_answers: stats.correct,
        total_correct_time_ms: stats.correctTimeMs,
        streak: finalMaxStreak,
      };
      updateUserStatsOnSupabase(user.id, statsDelta).then((res) => {
        if (res?.success && res.stats) {
          setUser(prev => {
            const newTotalGames = (prev.totalGamesPlayed || 0) + 1;
            const newSongsGuessed = (prev.totalSongsGuessed || 0) + stats.correct;
            const newWrongAnswers = (prev.totalWrongAnswers || 0) + stats.wrong;
            const newMeasuredCorrect = (prev.measuredCorrectAnswers || 0) + stats.correct;
            const newCorrectTime = (prev.totalCorrectTimeMs || 0) + stats.correctTimeMs;
            const totalAnswers = newSongsGuessed + newWrongAnswers;
            const computedAccuracy = totalAnswers > 0 ? Math.round((newSongsGuessed / totalAnswers) * 100) : 0;
            const computedAvgTime = newMeasuredCorrect > 0 ? Number((newCorrectTime / newMeasuredCorrect / 1000).toFixed(2)) : 0;

            const updated = {
              ...prev,
              totalScore: (prev.totalScore || 0) + finalGameScore,
