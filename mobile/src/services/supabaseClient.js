...
export function extractUserProfileFromAuth(supabaseUser, googleUser = null) {
  ...
  // Extract Name (try user_metadata, then googleUser fields, then email prefix fallback)
  const name =
    meta.full_name ||
    meta.name ||
    googleUser?.name ||
        ...
        if (idToken) {
          try {
            ...
            if (!error && data?.session?.user) {
              return { user: data.session.user, googleUser };
            }
            if (error) {
              console.warn('[Supabase Auth] ID Token sign-in notice:', error.message);
            }
          } catch (tokenErr) {
            console.warn('[Supabase Auth] ID token exchange exception:', tokenErr);
          }
        }
        return { user: googleUser, googleUser };
      } catch (nativeErr) {
        console.warn('[Native GoogleAuth] Native account chooser error:', nativeErr);
        const errMsg = String(nativeErr?.message || nativeErr || '');
        if (errMsg.toLowerCase().includes('cancel') || errMsg.includes('12501')) {
          return { error: 'Accesso annullato' };
        }
        return { error: `Accesso non riuscito: ${errMsg || 'Errore nativo'}` };
      }
    }
...
export async function deleteUserAccountFromSupabase(user) {
  ...
  try {
    ...
    // 1. Delete user from profiles (cascades to scores and user_stats)
    if (user.id) {
...
// --- SAVE / UPDATE USER PROFILE TO SUPABASE PROFILES TABLE ---
export async function saveUserProfileToSupabase(user) {
  ...
  try {
    ...
    // Persiste su user_stats i campi non coperti da Edge Function:
    // badge, death_parade_record, death_parade_points_record, note_streak.
    // Questi campi vengono scritti qui (lato client autenticato) come fallback cloud
    // per sopravvivere a disinstallazione/reinstallazione.
    try {
      const statsUpdate = {};

      if (Array.isArray(user.unlockedBadges) || Array.isArray(user.streakBadges)) {
        statsUpdate.badges = {
          unlocked: Array.isArray(user.unlockedBadges) ? user.unlockedBadges : [],
          streak: Array.isArray(user.streakBadges) ? user.streakBadges : [],
        };
      }

      const deathRecord = Number(user.deathParadeRecord || user.personalBests?.deathParade || 0);
      const deathPoints = Number(user.deathParadePointsRecord || user.personalBests?.deathParadePoints || 0);
      const noteStreak = Number(user.noteStreak || 0);

      // Solo aggiorniamo se abbiamo valori significativi (evita di sovrascrivere con 0)
      if (deathRecord > 0) statsUpdate.death_parade_record = deathRecord;
      if (deathPoints > 0) statsUpdate.death_parade_points_record = deathPoints;
      if (noteStreak > 0) statsUpdate.note_streak = noteStreak;

      if (Object.keys(statsUpdate).length > 0) {
        // Usa upsert con merge per non sovrascrivere altri campi
        await supabase
          .from('user_stats')
          .upsert(
            { user_id: userId, ...statsUpdate, updated_at: new Date().toISOString() },
            { onConflict: 'user_id' }
          );
      }
    } catch (_) {}
...
export async function saveScoreToSupabase(scoreData) {
  try {
    ...
    const clientDate = typeof window !== 'undefined'
      ? new Date().toLocaleDateString('sv') // Restituisce sempre YYYY-MM-DD nel fuso locale
...
// --- FETCH REALTIME GLOBAL LEADERBOARD (AGGREGATO DINAMICO DA SCORES & STATS) ---
export async function getLeaderboardFromSupabase() {
  try {
    ...
    // Fallback: interroga profiles se la vista non è disponibile
...
// --- FETCH INDIVIDUAL USER PROFILE (PROFILES + USER_STATS + STATISTICHE DERIVATE) ---
  ...
  try {
    ...
    if (!error && profileRow) {
      ...
      return {
        ...
        // Badge dal campo JSONB badges su user_stats (persistiti via saveUserProfileToSupabase)
        unlockedBadges: Array.isArray(statsObj?.badges?.unlocked) ? statsObj.badges.unlocked : [],
        streakBadges: Array.isArray(statsObj?.badges?.streak) ? statsObj.badges.streak : [],
      };
    }
...
export function subscribeToUserInbox(user, { onFriendRequestReceived, onFriendRequestResponse, onChallengeReceived, onChallengeCompleted, onTournamentReceived, onTournamentUpdated } = {}) {
  ...
  // 1. Canali Broadcast effimeri per messaggi veloci
  ...
  // 2. Canale Persistente Realtime (POSTGRES_CHANGES) per notifiche affidabili da database
  const userIdentifiers = [
    user.name?.trim().toLowerCase(),
    user.email?.trim().toLowerCase(),
    user.playerCode?.trim().toLowerCase(),
...
export async function sendAsyncChallengeToSupabase(challengePayload) {
  ...
  const processNotifications = (profile) => {
    ...
    // Le notifiche push FCM vengono gestite in modo autoritativo e affidabile
    // dal trigger database di PostgreSQL (trigger_async_challenges_webhook)
    // per evitare doppi invii e garantire il recapito anche se l'app si chiude.
  };
...
export async function notifyChallengeCompletedInSupabase(challengeId, resultPayload, challengerInfo) {
  ...
  if (challengerIdentifier) {
    ...
    // evitando doppie notifiche al vincitore/challenger.
    }).catch(() => {});
  }
}

// --- ASYNC MULTI-USER TOURNAMENT SYSTEM (48H TURN-BASED) ---
...
// --- SEND PUSH NOTIFICATIONS VIA SUPABASE EDGE FUNCTION (FCM V1) ---
