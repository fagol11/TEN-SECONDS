...
function isInsideZap(px, py) {
  if (pointInPoly(px, py, zapPolygon)) return true;
  // Add stroke thickness (0.025 in normalized coordinates)
  return distToZap(px, py) <= 0.022;
}

// 1. Full Icon Pixel Generator (Gradient + Dark Zap Logo)
function getFullIconPixel(x, y, width, height) {
  const nx = (x + 0.5) / width;
  const ny = (y + 0.5) / height;

  if (isInsideZap(nx, ny)) {
    return [9, 9, 13, 255]; // Dark slate #09090d
  }
...
// 2. Foreground Pixel Generator (Dark Zap on Transparent background)
...
console.log('Generating high quality app icon assets...');
